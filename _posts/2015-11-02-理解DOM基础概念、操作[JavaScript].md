...
上面的代码，如果给div1的style里面加上`position: relative`; 那么div3的`offsetParent`就变成了div1
...
我们通过`document.cookie`来获取当前网站下的`cookie`的时候，得到的是字符串形式的值，它包含了当前网站下所有的`cookie`。它会把所有的`cookie`通过一个分号+空格的形式串联起来。
